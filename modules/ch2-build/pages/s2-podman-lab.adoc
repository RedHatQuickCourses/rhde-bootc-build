...
--
+
You can see many processes that are NOT common in application containers, such as NetworkManager, DBus, and the SSH daemon.
...
--
+
Notice that the `ps` command is not available, but you can see from the `pstree` command that only the System daemon and Apache Web server are running.
Remember that most application containers do NOT include common troubleshooting commands and system services, so their container imges are leaner.
...
5. Because the HTML and JavaScript code of a singe-page web app is NOT supposed to be managed as data, improve your Containerfile to serve the application code from `/webapp` instead of from `/var/www`.
...
It uses a simple `sed` command search for `/var/www` and replace it by `/webapp`.
...
We will see, later in this course, how to test the bootc image in a way that makes evident that kind of issue.
