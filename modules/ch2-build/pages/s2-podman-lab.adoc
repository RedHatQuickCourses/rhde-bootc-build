...
--
+
See that the `ps` command is not available, but from the `pstree` command you can see that only the System daemon and Apache Web server are running.
Remember, that most application containers do NOT include common troubleshooting commands and system services, so their container iamges are leaner.
