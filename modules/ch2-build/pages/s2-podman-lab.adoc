...
Objective::
Create containerfiles and build bootc container images suitable to RHEL for edge systems
...
It installs the `httpd` package and enables the `httpd` service, them adds the files that mock a singe-page web application.
+
...
--
+
There are many processes that are NOT common in application containers, such as NetworkManager, DBus, and the SSH daemon.
...
+
... It does NOT send logs to the standard output, so those logs are NOT visible with `podman logs` and would also NOT be visible if run from a Kubernetes cluster.
+
... It runs as `root`, because it does not set a `USER`.
+
... It listens to a privileged TCP port (80), as per the default Apache Web Server configuration. It should listen to an unprivileged port, to not requile elevated privileges when run in a Kubernetes cluster.
+
...
It also runs fewer Systemd services, because it does not inherit Systemd configurations from the bootc base container image.
...
--
$ *curl 127.0.0.1:8080*
This would be the HTML and Javascrit code of your IMPROVED single-page web application.
--
