...
You will review a containerfile which configures different aspects of a RHEL system, and build a bootc container image from it.
...
.. If needed, create a local clone of the samples code repository of this course, and make it your working directory.
+
...
+
WARNING: While it is possible to run a privileged, rootful container with access to host namespaces, and thus affect system and kernel settings, running a bootc container image this way could have unexpected and potentially dangerous effects on its host, in the case your _development machine_.
+
[source,subs="verbatim,quotes"]
--
bash-5.2# *exit*
