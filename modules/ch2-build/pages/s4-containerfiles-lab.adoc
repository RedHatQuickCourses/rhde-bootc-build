...
You will build a containerfile which configures different aspects of a RHEL system.
You will test it as an application container, and will find no evidence that those aspects were actually configured, highlighting the need for system testing of your bootc container images.
...
+
WARNING: While it is possible to run a privileged, rootful container with access to host namespaces, and thus affect system and kernel settings, running a bootc container this way could potentially damage the configurations of its host, in the case your _development machine_.
+
[source,subs="verbatim,quotes"]
--
bash-5.2# *exit*
