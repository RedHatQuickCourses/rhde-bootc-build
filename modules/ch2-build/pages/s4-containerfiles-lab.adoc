...
Objective::
Create containerfiles that include system configurations and demonstrate that they CANNOT be effectively tested by using Podman.
...
If you are using the course classroom, you log in on the `workstation` VM as the user `student` with password `student`.
...
You will review a containerfile that configures different aspects of a RHEL system, and build a bootc container image from it.
Then you will test it as an application container, and will find no evidence that those aspects were actually configured, highlighting the need for system testing of your bootc container images.
...
2. Inspect the containerfile, which configures Kernel settings, kernel command-line arguments, and Firewalld for a physical machine that would be installed from that bootc container image.
...
.. Review its kernel configuration files.
+
...
.. Review its kernel command-line configuration files.
+
...
+
WARNING: While it is possible to run a privileged, rootful container with access to host namespaces, and thus affect system and kernel settings, running a bootc container image this way could have unexpected and potentially dangerous effects on its host, in this case, your _development machine_.
+
[source,subs="verbatim,quotes"]
--
bash-5.2# *exit*
...
5. Stop and remove your test container, so it does not keep port 8080 busy on your _development machine_.
+
[source,subs="verbatim,quotes"]
--
$ *podman stop httpd*
