...
.. Bootc container images include the bootable container system.
+
[source,subs="verbatim,quotes"]
--
$ podman run -it --rm --name bootc docker://registry.lab.example.com:5000/rhel10/rhel-bootc bash -c "rpm -q bootc"
...
bootc-1.1.6-2.el10_0.x86_64
$ podman run --rm -it --name ubi docker://registry.lab.example.com:5000/ubi10/ubi bash -c "rpm -q bootc"
..
...
[source,subs="verbatim,quotes"]
--
$ podman run -it --rm --name bootc docker://registry.lab.example.com:5000/rhel10/rhel-bootc bash -c "rpm -q kernel"
...
kernel-6.12.0-55.24.1.el10_0.x86_64
$ podman run --rm -it --name ubi docker://registry.lab.example.com:5000/ubi10/ubi bash -c "rpm -q kernel"
..
package kernel is not installed
--

// 5. Install the RHEL virtualization tools -- may not need in this course
