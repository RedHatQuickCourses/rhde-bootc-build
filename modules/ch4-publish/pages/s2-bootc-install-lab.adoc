...
Also make a not if and its IP address, default gateway, and DNS server address.
...
Remember that previous registry login were done on your _developer machine_ from your unprivileged user.
+
[source,subs="verbatim,quotes"]
--
...
... The `--skip-fetch` option is required, else the `bootc install` command would try access the private registry, but would fail because it has no TLS certificate nor credentials.
...
... The `--kargs` option provide static network configuration.
+
[source,subs="verbatim,quotes"]
--
...
5. If you perceive you did a mistake, there are a few approaches you can use to try it again, using the provided virtual labs environment:
+
IMPORTANT: if all went well so far, you can skip this entire step.
...
6. On your _developer machine_, verify that your _test machine_ runs the `httpd-system` bootc container image and open a nmew SSH session to your _test machine_
...
+
If you do not remember about those settings, refer to the xref:ch2-build:s4-containerfiles-lab.adoc[previous lab] where you build that image and verified that its settings were not applied with Podman.
...
This was the final lab in this course, but the next course of this learning approach shows how to use the *bootc image builder* utility to create customized installation medias for edge devices and also how to create customized boot disk images for clouds and hypervisors.