...
If you are using the course classroom, log in on the `workstation` VM, as your _development machine_, using the user `student` with password `student`.
...
You will publish a bootc container image in a private registry, in a way that emulates the results of a CI/CD pipeline, and you will reinstall an existing system from that container image, as an alternative approach to perform system testing.
...
.. Copy your bootc container image to the private registry and set a fixed tag which resembles a build number, so it remains unique as you perform new builds of the same image.
+
[source,subs="verbatim,quotes"]
--
...
--
+
NOTE: Alternatively, you can open a web terminal directly on the `servera` machine, using the ROLE web interface.
Notice that the web terminal will stop working after you reinstall your _test machine_.
...
--
+
NOTE: In most circunstances, you would run the `bootc` command from a bootc container image instead of directly from a system installed using package mode.
...
You can use the Network Manager CLI to get that information and more.
+
[source,subs="verbatim,quotes"]
--
$ *nmcli device status*
...
.. Make a note that the network configuration is manual, and the primary network interface is the `ens3` device.
Also make a note of its IP address, default gateway, and DNS server address.

4. Re-install your _test machine_ from a bootc container image.

.. Install the public CA certificate of your private container registry, so you do NOT need to disable TLS validation when accessing its container images.
+
[source,subs="verbatim,quotes"]
--
...
.. Log in on your private registry, but from the `root` user.
Remember that previous registry logins were done on your _developer machine_ from your unprivileged user.
+
[source,subs="verbatim,quotes"]
--
...
.. Review the `bootc-install.sh` script.
...
... The `--skip-fetch` option is required, or else the `bootc install` command would try to access the private registry, but would fail because it has no TLS certificate or credentials.
... The `--acknowledge-destructive` option skips a wait time before starting the installation.
... The `--replace alongside` option preserve the old contents of the existing system root, which could be useful for troubleshooting.
...
.. Verify that the system is now setup to boot from a bootc container image, that is, from an ostree deployment on its root disk.
+
[source,subs="verbatim,quotes"]
--
$ *bootc status*
...
5. If you perceive you made a mistake, there are a few approaches you can use to try it again, using the provided virtual labs environment:
+
IMPORTANT: If all went well so far, you can skip this step.

.. BEFORE you reboot your _test machine_, you can run the `bootc install` command, adding the `--stateroot tryagain` option, where `tryagain` is any name you wish to assign to your new bootc system installation, other than `default`.

.. If you only perceive a mistake AFTER reboot, you can retry using the `serverb` VM as your _test machine_.
Just remember to replace the IP address of the `servera` VM (172.25.250.10) with the IP address of the `serverb` VM (172.25.250.11) on your `bootc-install.sh` script.
+
If you do so, remember to replace, in the remaining steps of this lab, the `servera` hostname name with the `serverb` hostname.

.. If you want to try again, but after having already rebooted both `servera` and `serverb` VMs, you must delete and re-create your ROLE classroom, and then perform the previous labs that prepare your _development machine_ and build the `httpd-system` bootc container images.
...
6. On your _developer machine_, verify that your _test machine_ runs the `httpd-system` bootc container image and open a new SSH session to your _test machine_
...
+
If you do not remember those settings, refer to the xref:ch2-build:s4-Containerfiles-lab.adoc[previous lab] where you built that image and verified that its settings were not applied with Podman.

.. Verify that it now reports as having been deployed using bootc.
+
[source,subs="verbatim,quotes"]
--
# *bootc status*
...
7. On your _development machine_, promote your bootc container image as vetted for use by actual edge devices.
...
--
+
Notice that the `skopeo copy` command above did NOT actually copy any layers, because they already exist on the container registry, from the previous copy operation.
...
--
+
NOTE: A CI/CD system would probably use the second style of copy, from remote to remote, to set all floating tags, instead of making multiple copies from local to remote, as you did in the previous step.
This activity uses both styles to demonstrate that it makes no difference to the final result.
...
--
+
NOTE: Your digests will be different from the output above, because Containerfile builds are NOT reproducible. 
If you build an OCI container twice, from exactly the same inputs, you get different image layers and a different image manifest.
There is ongoing work on the Podman community and OCI standards to create a way of performing reproducible container image builds.
...
This is the final lab in this course, but the next course if this learning approach covers how to use the *bootc image builder* utility to create customized installation medias for edge devices and also how to create customized boot disk images for clouds and hypervisors.