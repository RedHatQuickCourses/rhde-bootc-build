...
Objective::
Troubleshoot and fix a bootc container image which works in a container engine but fails in a local VM.
...
This time, you will find that the system does NOT works as expected.
You will then find the root cause of the failure, fix the containerfile, create another VM from the updated bootc container image, and verify that it now works as expected.
