...
6. *Optional:* you do not need to build a new test VM every time you build a new bootc container image.
You can upgrade an existing VM, taking advantage of the virtio device configured by the `virt-install` command.
...
.. Open a SSH session to your running test VM, stage your updated bootc container image, and reboot your test VM.
+
[source,subs="verbatim,quotes"]
--
$ *ssh -i edge-key -p 8022 core@127.0.0.1*
...
7. If you wish, you can now delete your test VM and temporary directories, especially the large OCI directory which now contains hunders of MB of bootc container image layers.
+
[source,subs="verbatim,quotes"]
--
$ *virsh destroy bootc-test*
