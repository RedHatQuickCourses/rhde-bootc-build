...
Objective::
Deploy a bootc container image, from the local containers storage, in a local virtual machine (VM).
...
You will use the standard RHEL installation media, with a custom kickstart script, to install RHEL in a local VM from a bootc container image.
Thanks to the direct kernel loading feature of libvirt, and also to virtio devices backed by user directories, you do NOT require PXE boot, HTTP servers, or custom installation media.
...
+
It is a standard RHEL Anaconda configuration for unattended installation, which uses a number of features:
+
... A virtiofs mount, in a `%pre` script, to get the contents of your bootc container image.
... The `ostreecontainer` command, to set a bootc container image as the installation source.
... A `%post` script which creates an initial user, with a known password and SSH key.
... Also in the `%post` script, it sets an initial hostname, which helps you identify your test VM and recognize it didn't go into any day-2 configuration or on-boarding process yet.
+
...
It is also designed to require no access to a remote container registry, by using the following features of libvirt:
+
...
3. Create a local VM from your bootc container images, using the provided scripts.

.. Run the installation script.
+
You could use it from a machine for which you do not have root access, as long as someone already installed Libvirt and its tooling.
+
[source,subs="verbatim,quotes"]
--
...
+
The message, which states `Deployment starting`, indicates when Anaconda is done preparing the system and starts reading your bootc container image:
+
[source,subs="verbatim,quotes"]
--
...
...
.. If all goes well, you will see a message which states that your VM (_domain_ or _guest_ in libvirt's jargon) was created, but you may not see its RHEL login screen on the terminal.
+
[source,subs="verbatim,quotes"]
--
...
...
4. Verify that your test VM was actually provisioned from the contents of your bootc container image.
...
--
+
Notice the much higher number of running processes, compared to the previous lab, where you run the same bootc container image from Podman. Also notice processes such as terminal login managers, a Cron daemon, and additional Systemd services you did not see running from Podman.
...
5. Verify that your _development machine_ can access the single-page web application running in the test VM.

.. Verify that you get, through the libvirt port-forward tunnel, the contents of your single-page web application.
+
[source,subs="verbatim,quotes"]
--
$ *curl 127.0.0.1:8080*
...
If you want to keep it, you must edit your scripts to change the name of the VM you will create in the next activity.
+
[source,subs="verbatim,quotes"]
--
$ *virsh destroy bootc-test*
...
The next activity walks through troubleshooting a bootc container image that fails to run correctly in a VM and fixing the issue by updating its containerfile and building a new bootc image.
